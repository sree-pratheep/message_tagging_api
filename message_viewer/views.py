...
def add_message_tags(request, pk):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'message_viewer/telegrammedia_form.html', {'form': form})
    else:
        media = TelegramMedia.objects.get(pk=pk)
...
def search_messages(request, searched=None):
    ...
    return render(request, 'message_viewer/search_results.html', {'page_obj': page_obj, 'searched': searched})


class TelegramMessageListView(ListView):
    model = TelegramMessage
    template_name = 'message_viewer/home.html'
...
class TelegramMessageFilterListView(BaseFilterView, ListView):
    template_name = 'message_viewer/filter_list.html'
    model = TelegramMessage
